...
\\
That simple a network can only transform two input numbers, say (0.1, 0.9), to two output numbers, say (0.3, 0.4). In the Open Office Calc document \textit{Simple\_Neural\_Net.ods} such a network is implementet in a spreadsheet.
\\
\\
...
To train the network we need a measure of how far the output values $\vec a^{(2)}$ are from the target values $\vec T$. As mentioned this measure is called \textit{the cost function}.
\\ 
